...
from autocub_mcp.client import (
    APIClient,
    APIError,
    AuthError,
    ForbiddenError,
...
async def client(monkeypatch):
    ...
    monkeypatch.setattr("autocub_mcp.client.asyncio.sleep", AsyncMock())
    yield instance
...
async def test_byok_does_not_persist(client):
    ...
    assert await client.get("/v1/cub/latest", api_key="fixture-key") == {"value": "1.2300"}
    client._client.request.assert_awaited_with(
        "GET", "/v1/cub/latest", params=None, json=None, headers={"X-API-KEY": "fixture-key"}
    )
    await client.get("/v1/cub/latest")
...
async def test_post(client):
    ...
    assert (
        await client.post("/v1/calc/area", json=[{"area_real_m2": "1.20"}], api_key="fixture-key")
        == []
    )
...
async def test_mapping_no_retry(client, status, error, caplog):
    ...
    with pytest.raises(error) as exc:
        await client.get("/v1/cub/latest", api_key="fixture-key")
...
async def test_retry_then_success(client, status):
    ...
    assert await client.get("/v1/cub/latest") == {}
